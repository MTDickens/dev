...
split s = getFirst s ""
  where
    getFirst (' ' : xs) first = Just (first, xs)
    getFirst (x : xs) first = getFirst xs (first ++ [x])
...
checkNumber (a, b) = if checkNumberHelper a && checkNumberHelper b then Just (a, b) else Nothing
  where
    checkNumberHelper = all (`notElem` "0123456789")
...
count x = modify (add x []) -- modify the state
  where
    add i prev ((j, cnt) : xs) =
      if i == j
        then prev ++ ((j, cnt + 1) : xs)
        else add i (prev ++ [(j, cnt)]) xs
    add i prev _ = prev ++ [(i, 1)]
